...
module.exports = async function pull(componentIdOrName) {
  ...
  if (!config.platform || !config.projectId || !config.accessKey) {
  ...
  // Ensure client/src/components directory exists
  const componentsDir = path.join(process.cwd(), 'client', 'src', 'components');
  if (!fs.existsSync(componentsDir)) {
    fs.mkdirSync(componentsDir, { recursive: true });
    console.log(chalk.gray('Created client/src/components directory'));
  }

  // Generate component file content with metadata
  const fileContent = `// vibe-source: ${config.platform}/api/apps/${config.projectId}/entities/Component/${component.id}
...
${component.tsx_code || component.code}`;
  ...
  try {
    ...
    console.log(chalk.green(`✅ Component saved to client/src/components/${fileName}`));
